...
import { useRouter, useReducer } from "next/dist/client/router";
...
export default function App() {
    const [alertState, alertDispatch] = useReducer(alertReducer, initialArg, init);
    ...
    const ethLogin = async (e) => {
        try {
            ...
            if (!loginResponse.success) {
                setTimeout(() => { setShowWalletConnect(false) }, 5000)
                setShowWalletConnect("failed")
            }

            if (loginResponse.success) {
                setTimeout(() => { setShowWalletConnect(false) }, 5000)
                setShowWalletConnect("successful")
    ...
    const solLogin = async (e) => {
        try {
            ...
            if (!loginResponse.success) {
                setTimeout(() => { setShowWalletConnect(false) }, 3000)
                setShowWalletConnect("failed")
            }
    ...
    return (
        <div className="flex flex-col">
            {!showWalletConnect ? <span /> : showWalletConnect === "successful" ? <LoginAlert success message="Login Successful" /> : <LoginAlert message="Login Failed" />}
