                    ...
                    <div id="card" className="flex flex-col justify-center rounded-xl border border-gray-300 shadow-sm p-3 pt-6 sm:p-6 bg-white">
                        ...
                        {active == 3 ? ethNFTs.map(
                            (nft, index) => {
                                ...
                                return (<div><video onLoad={(e) => { e.target.style.display = "block" }} style={{ "display": "none" }} width="400" loop muted onError={(e) => { e.target.style.display = "none" }}>
                                    <source src={nft} type='video/webm' />
                                </video>
                                    <img onLoad={(e) => { e.target.style.display = "block" }} style={{ "display": "none" }} key={index} onError={(event) => { event.target.style.display = 'none' }} src={nft} width="400"></img></div>)
                            }
                        ) : ""}
