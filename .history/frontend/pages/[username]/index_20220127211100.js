        ...
        async function fetchEthNFTs() {
            try {
                ...
                for (let i = 0; i < data.nfts.length; i++) {
                    ethNFTArray.push({ url: data.nfts[i].file_url, name: data.nfts[i].name, contractAddress: data.nfts[i].contract_address })
                }
            ...
            <div className="flex flex-col align-items-center items-center mt-16">
                ...
                {loading ? <Loading /> : accountExists ?
                    <div id="card" className="flex flex-col justify-center rounded-xl border border-gray-300 shadow-sm p-3 pt-6 sm:p-6 bg-white">
                        ...
                        {active == 3 ? ethNFTs.map(
                            (nft, index) => {

                                return (<div className="m-4 mx-6"><video onLoad={(e) => { e.target.style.display = "block" }} style={{ "display": "none" }} width="300" loop muted onError={(e) => { e.target.style.display = "none" }}>
                                    ...
                                    <div className="flex flex-row">
                                        <CustomLabel>{nft.name}</CustomLabel>
                                        <a href={`https://opensea.io/assets/0x57f1887a8bf19b14fc0df6fd9b2acc9af147ea85/${nft.contractAddress}`}>
