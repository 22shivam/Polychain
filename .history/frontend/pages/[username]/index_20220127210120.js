        ...
        async function fetchEthNFTs() {
            try {
                ...
                for (let i = 0; i < data.nfts.length; i++) {
                    ethNFTArray.push(data.nfts[i].file_url)
                }
                ethNFTArray = ethNFTArray.filter((c, index) => {
                    return ethNFTArray.indexOf(c) === index && c !== "";
                });
                setEthNFTs(ethNFTArray)
            } catch (e) {
                console.log(e)
                        ...
                        {active == 3 ? ethNFTs.map(
                            (nft, index) => {
                                ...
                                return (<div className="m-4 mx-6"><video onLoad={(e) => { e.target.style.display = "block" }} style={{ "display": "none" }} width="300" loop muted onError={(e) => { e.target.style.display = "none" }}>
                                    ...
                                    <img onLoad={(e) => { e.target.style.display = "block" }} style={{ "display": "none" }} key={index} onError={(event) => { event.target.style.display = 'none' }} src={nft} width="300"></img></div>)
                            }
                        ) : ""}
