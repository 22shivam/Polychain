...
export default function UserPayment() {
    ...
    useEffect(() => {
        ...
        async function fetchEthNFTs() {
            try {
                let response = await fetch('https://api.nftport.xyz/v0/accounts/0x001FbCD3Ca9720Bc35762eAbf82821f05d4d3b02?chain=ethereum', {
                    headers: {
        ...
        async function fetchPolygonNFTs() {
            try {
                let response = await fetch('https://api.nftport.xyz/v0/accounts/0x001FbCD3Ca9720Bc35762eAbf82821f05d4d3b02?chain=polygon', {
                    headers: {
                        "Authorization": "c4688209-7089-4421-add7-e2904a27de41",
                        "Content-Type": "application/json"
                    }
            ...
            <div className="flex flex-col align-items-center items-center mt-16">
                ...
                {loading ? <Loading /> : accountExists ?
                    <div id="card" className="flex flex-col justify-center rounded-xl border border-gray-300 shadow-sm p-3 pt-6 sm:p-6 bg-white">
                        ...
                        {ethNFTs.map(
                            (nft, index) => {
                                return <img key={index} src={fixUrl(nft)} width="80"></img>
                            }
                        )}
