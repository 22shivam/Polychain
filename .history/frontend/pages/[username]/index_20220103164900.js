...
export default function UserPayment() {
    ...
    useEffect(() => {
        ...
        const coinbaseResponse = (async () => {
            if (selectedCurrency.name === "ETH") {
                return (await (await fetch(COINBASE_URL_ETH).json()))
            } else if (selectedCurrency.name === "SOL") {
                return (await (await fetch(COINBASE_URL_SOL).json()))
            } else if (selectedCurrency.name === "BTC") {
                return (await (await fetch(COINBASE_URL_BTC).json()))
            }
        })();
