                ...
                {loading ? <Loading /> : accountExists ?
                    <div id="card" className="flex flex-col justify-center rounded-xl border border-gray-300 shadow-sm p-3 pt-6 sm:p-6 bg-white">
                        <nav className="flex flex-row justify-evenly">
                            <CustomLabel style={{ fontSize: "1.2rem" }} onClick={() => { setActive(1) }} className={active == 1 ? "text-brand-primary-medium mb-8" : "text-gray-500"}>Payment</CustomLabel>
                        ...
                        {ethNFTs.map(
                            (nft, index) => {
                                ...
                                return (<div><video width="80" loop muted onError={(e) => { e.target.style.display = "none" }}>
                                    <source src={nft} type='video/webm' />
                                </video>
                                    <img key={index} onError={(event) => { event.target.style.display = 'none' }} src={nft} width="80"></img></div>)
                            }
                        )}
