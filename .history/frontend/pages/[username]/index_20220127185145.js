...
export default function UserPayment() {
    ...
    useEffect(() => {
        ...
        const fetchEthNFTs = async () => {
            const response = await fetch(`${MORALIS_ETH_NFT_URL}/${ETHAddress}/nft?chain=eth&format=decimal`, {
                method: 'GET',
                headers: {
                    "X-API-Key": "WpUzA6KyUb5R29iYyaOuiXwnQl68ZcfyQhRxMujqBMD8IutAIrDqzU1l1jlay2Pc",
                    "accept": "application/json"
                },
            })
            const data = await response.json()
            console.log(data)
            setEthNFTs(data.data)
        }
