        ...
        <div className="flex flex-col w-screen h-screen">
            ...
            {userAccount.address ? hasUsername ?
                <div className="flex flex-row items-stretch flex-grow">
                    {/* sidebar  */}
                    <nav className='shadow-md w-64 absolute inset-y-0 left-0 transform -translate-x-full transition duration-200 ease-in-out'>
                        <ul className="flex flex-col mt-2 px-2">
                            {navigationItems.map((item) => {
                                let className = "transition duration-200 px-2 rounded-md py-2 font-semibold hover:bg-gray-50 text-gray-600" + (item.id == selectedNav ? " bg-gray-100" : "")
                                return (
                    ...
                    {/* content */}
                    {selectedNav == 1 ? <main id="dashboard" className='flex flex-col mt-4 mb-20 ml-8 p-10 flex-1'>
