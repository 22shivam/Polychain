...
export default function App() {
    ...
    const solLogin = async (e) => {
        try {
            ...
            if (!loginResponse.success) {
                setTimeout(() => { showWalletConnect(false) }, 5000)
                showWalletConnect("failed")
            }

            if (loginResponse.success) {
                setTimeout(() => { showWalletConnect(false) }, 5000)
                showWalletConnect("successful")
    ...
    return (
        <div className="flex flex-col">
            {!showWalletConnect ? <span /> : showWalletConnect === "successful" ? <LoginAlert success="true" message="Login Successful" /> : <LoginAlert success="false" message="Login Failed" />}
