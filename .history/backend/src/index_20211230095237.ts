...
app.post("/userDetails/update", (req: any, res: any) => {
    ...
    jwt.verify(token, process.env.COOKIE_SECRET as string, async (err: any, decoded: any) => {
        if (err) { // not logged in
            ...
            const { ETHAddress, BTCAddress, DESOAddress, SOLAddress, bio, profilePic } = req.body
                ...
                if (!user) { // no username for logged in acct
                    ...
                    if (ETHAddress != address) { // primary eth address changed )
                        ...
                        if (user) { // new primary eth address is already taken
                            ...
                            if (user && user.ETHAddress !== address) { // sol address is already taken by SOMEONE ELSE
                                return res.json({
                                    success: "false",
                ...
                if (!user) { // no username for logged in acct
                    ...
                    if (SOLAddress != address) { // primary sol address changed )
                        ...
                        if (user) { // new primary sol address is already taken
                            ...
                            if (user && user.SOLAddress !== address) { // eth address is already taken by SOMEONE ELSE
                                return res.json({
                                    success: "false",
                                    message: "Ethereum address is already associated with another Username"
                                })
            ...
            // check if these addresses (only do this for SOL and ETH as they are being used for authentication) already exist in the database -- if they do then return success failed
            if (blockchain === "eth") {
                ...
                // verify if solana address not already used
                let user = await User.findOne({ SOLAddress: SOLAddress })
                // this is to make sure that we dont throw an error if the account it is associated with is the current acct
    ...
    if (blockchain == "eth") {
        ...
        const user = await User.findOne({ SOLAddress: address })
        if (user) {
            user.ETHAddress = ETHAddress
            user.BTCAddress = BTCAddress
            user.DESOAddress = DESOAddress
