    ...
    try {
        ...
        jwt.verify(token, process.env.COOKIE_SECRET as string, async (err: any, decoded: any) => {
            ...
            if (blockchain === "eth") {
                ...
                const resp = await axios.get(`https://api.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=asc&apikey=${process.env.ETHERSCAN_API_KEY}`);
                console.log(resp)
