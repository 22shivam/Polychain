    ...
    jwt.verify(token, process.env.COOKIE_SECRET as string, async (err: any, decoded: any) => {
        if (err) { // not logged in
            ...
            if (bio && bio.length > 160) {
                return res.json({
                    success: false,
                    message: "Bio must be less than 160 characters backend"
                })
            }
...
let logoutrequestcounter = 0
let errorjwt = 0

app.get("/logout", async (req, res) => {
    console.log("logout requesr resceived", logoutrequestcounter)
    logoutrequestcounter++
...
app.get("/isLoggedIn", async (req, res) => {
    ...
    jwt.verify(token, process.env.COOKIE_SECRET as string, async (err: any, decoded: any) => {
        if (err) {
            console.log(token)
            console.log("error verifying jwt", errorjwt)
            errorjwt++
