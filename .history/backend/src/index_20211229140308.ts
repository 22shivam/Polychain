...
app.post("/login/sol", async (req, res) => {
    ...
    const isValid = nacl.sign.detached.verify(message, signature.signature, signature.publicKey)
    console.log("signer", isValid)
    // generate jwt send it as response
    const token = jwt.sign({
        data: { address: signature.publicKey, blockchain: "sol" }
