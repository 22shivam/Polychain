...
app.post("/login/sol", async (req, res) => {
    ...
    console.log(address)
    const isValid = nacl.sign.detached.verify(encodedMessage, new Uint8Array(signature.data), new Uint8Array(address.toBytes()))
    // TODO: fix this jwt gen of solana man
    // generate jwt send it as response 
    const token = jwt.sign({
        data: { address: address.toString(), blockchain: "sol" }
