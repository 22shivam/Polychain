...
import checkIfUsernameAvailable from "./utils/checkIfUsernameAvailable";
...
const SOLANA_EXPLORER_URL: string = process.env.SOLANA_EXPLORER_URL!
...
app.post("/register/sol", async (req, res) => {
    ...
    await checkIfUsernameAvailable(username, res)
    ...
    const existingUser = await User.findOne({ SOLAddress: from }).exec()
    if (existingUser) {
        return res.json({
            success: false,
...
app.post("/register/eth", async (req, res) => {
    ...
    let existingUser = await User.findOne({ ETHAddress: txDetailsFromEtherscan.from.toUpperCase() }).exec()
    if (existingUser) {
        return res.json({
            success: false,
            message: "Address already used"
