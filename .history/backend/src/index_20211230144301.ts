...
app.post("/login/sol", async (req, res) => {
    const { signature, message, address, toString, toBytes } = req.body
    const encodedMessage = new TextEncoder().encode(message);
    console.log(address.data)
    const isValid = nacl.sign.detached.verify(encodedMessage, new Uint8Array(signature.data), new Uint8Array(address.data.toBytes()))
    // TODO: fix this jwt gen of solana man
    // generate jwt send it as response 
    const token = jwt.sign({
        data: { address: address.data.toString(), blockchain: "sol" }
