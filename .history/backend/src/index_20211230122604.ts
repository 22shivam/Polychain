...
app.post("/login/sol", async (req, res) => {
    ...
    const isValid = nacl.sign.detached.verify(encodedMessage, new Uint8Array(signature.data), new Uint8Array(address.toBytes().data))


    console.log(address.toString())
    // generate jwt send it as response
    const token = jwt.sign({
        data: { address: address.toString().toUpperCase(), blockchain: "sol" }
