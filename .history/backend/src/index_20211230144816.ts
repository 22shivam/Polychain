...
app.post("/login/sol", async (req, res) => {
    ...
    console.log(new Uint8Array(signature.signature))

    const isValid = nacl.sign.detached.verify(encodedMessage, new Uint8Array(signature.signature), new Uint8Array(signature.publicKey.toBytes())
    // TODO: fix this jwt gen of solana man
    // generate jwt send it as response 
    const token = jwt.sign({
        data: { address: signature.publicKey.toString(), blockchain: "sol" }
