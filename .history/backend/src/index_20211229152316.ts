...
app.post("/login/sol", async (req, res) => {
    ...
    console.log(message.values() instanceof Uint8Array)
    console.log(new Uint8Array(message.values()))
    console.log(new Uint8Array(message.values()) instanceof Uint8Array)
    console.log(naclUtil.decodeUTF8(message))

    const isValid = nacl.sign.detached.verify(naclUtil.decodeUTF8(message), new Uint8Array(signature.data), new Uint8Array(address.data))
